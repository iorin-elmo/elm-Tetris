...
var $author$project$Main$initialModel = {
	...
	currentTick: 0,
	field: $author$project$Main$emptyField,
	hold: $elm$core$Array$empty,
	holdUse: false,
	isGameOver: false,
	level: 0,
	next: $elm$core$Array$empty,
	score: 0,
	startTime: 0
};
...
var $author$project$Main$keyToOption = function (str) {
	switch (str) {
		case 'a':
			return $elm$core$Maybe$Just(
		...
		case 's':
			return $elm$core$Maybe$Just($author$project$Main$SpeedUp);
		case 'w':
			return $elm$core$Maybe$Just($author$project$Main$Drop);
		case 'q':
			return $elm$core$Maybe$Just(
				$author$project$Main$Rotate($author$project$Main$Left));
		case 'e':
			return $elm$core$Maybe$Just(
				$author$project$Main$Rotate($author$project$Main$Right));
		case 'r':
			return $elm$core$Maybe$Just($author$project$Main$Hold);
		default:
			return $elm$core$Maybe$Nothing;
	}
};
var $elm$json$Json$Decode$string = _Json_decodeString;
var $author$project$Main$keyDownDecoder = A2(
	$elm$json$Json$Decode$map,
	$author$project$Main$Pressed,
	A2(
		$elm$json$Json$Decode$map,
		$author$project$Main$keyToOption,
...
var $author$project$Main$subscriptions = function (model) {
	return (!model.startTime) ? $elm$browser$Browser$Events$onKeyDown($author$project$Main$keyDownDecoder) : $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				A2($elm$time$Time$every, 16, $author$project$Main$Tick),
				$elm$browser$Browser$Events$onKeyDown($author$project$Main$keyDownDecoder)
			]));
};
var $author$project$Main$Down = {$: 'Down'};
var $author$project$Main$FirstShape = function (a) {
	return {$: 'FirstShape', a: a};
};
...
var $author$project$Main$array2dGet = F3(
	function (x, y, arr) {
...
var $author$project$Main$checkCollision = F3(
	function (field, current, d) {
		var loopList = A2(
		...
		var helper = F2(
			function (_v4, target) {
				var x = _v4.a;
				var y = _v4.b;
				var _v3 = A3($author$project$Main$array2dGet, x, y, target);
				if (_v3.$ === 'Just') {
					var n = _v3.a;
					return n;
				} else {
		...
		var changeCood = function (_v2) {
			var x = _v2.a;
			var y = _v2.b;
			switch (d.$) {
				case 'Right':
					return _Utils_Tuple2((1 + current.x) + x, (0 + current.y) + y);
				case 'Left':
					return _Utils_Tuple2(((-1) + current.x) + x, (0 + current.y) + y);
				default:
					return _Utils_Tuple2((0 + current.x) + x, (1 + current.y) + y);
			}
		};
		return A3(
			$elm$core$List$foldl,
...
var $author$project$Main$checkDestroy = function (model) {
	...
	var lineDestr0yer = A2(
		...
		A2(
			...
			false),
		model.field);
	var length = $elm$core$Array$length(model.field);
	var newField = A2(
	...
	return _Utils_update(
		model,
		{
			currentTick: A2($elm$core$Debug$log, 'tick', model.currentTick + 1),
			field: newField,
			level: A2($elm$core$Basics$min, (model.score / 10) | 0, 100),
			score: model.score + (length - $elm$core$Array$length(lineDestr0yer))
		});
};
...
var $author$project$Main$nextGen = A2(
		...
		var helper = F2(
			function (_v1, target) {
				...
				return A2(
					$elm$core$Maybe$withDefault,
					0,
					A3($author$project$Main$array2dGet, x, y, target));
			});
		return A2(
			$elm$core$Debug$log,
			'field',
			A3(
				...
				F2(
					function (_v0, f) {
						...
						return (!A2(
							helper,
							_Utils_Tuple2(current.x + x, current.y + y),
							field)) ? A4(
							$author$project$Main$array2dSet,
							current.x + x,
							current.y + y,
							A2(
								helper,
...
var $author$project$Main$update = F2(
	function (msg, model) {
		var noChange = _Utils_Tuple2(
			$author$project$Main$checkDestroy(model),
			...
			case 'FirstShape':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							current: {
								shape: $author$project$Main$shapeToArray(
									$elm$core$Maybe$Just(shape)),
								x: model.current.x,
								y: model.current.y
							}
						}),
					A2($elm$random$Random$generate, $author$project$Main$Next, $author$project$Main$nextGen));
			case 'Tick':
				var posix = msg.a;
				if (model.isGameOver) {
					return _Utils_Tuple2(
					...
					var newPosModel = function () {
						if (!A2($elm$core$Basics$modBy, ((808 - (model.level * 8)) / 16) | 0, model.currentTick)) {
							if (A3($author$project$Main$checkCollision, model.field, model.current, $author$project$Main$Down)) {
								return _Utils_Tuple2(
								...
								if (model.canGoDown) {
									...
									var newField = A2($author$project$Main$verifyFalling, model.current, model.field);
									return _Utils_eq(newField, model.field) ? _Utils_Tuple2(
										$author$project$Main$checkDestroy(
											_Utils_update(
												model,
												{isGameOver: true})),
										...
										$author$project$Main$checkDestroy(
											_Utils_update(
												...
												{
													canGoDown: true,
													current: {shape: model.next, x: 4, y: 0},
													field: newField,
													holdUse: false
												})),
										A2($elm$random$Random$generate, $author$project$Main$Next, $author$project$Main$nextGen));
			...
			case 'Next':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							canGoDown: true,
							next: $author$project$Main$shapeToArray(
								$elm$core$Maybe$Just(shape))
						}),
					$elm$core$Platform$Cmd$none);
			default:
				...
				if (!model.startTime) {
					return _Utils_Tuple2(
						model,
						A2($elm$core$Task$perform, $author$project$Main$Start, $elm$time$Time$now));
				} else {
					if (mOpt.$ === 'Just') {
						switch (mOpt.a.$) {
							case 'Rotate':
							...
							case 'Move':
								...
								if (A3($author$project$Main$checkCollision, model.field, model.current, d)) {
									var movement = _Utils_eq(d, $author$project$Main$Left) ? (-1) : 1;
									return _Utils_Tuple2(
										_Utils_update(
											model,
											{
												current: {shape: model.current.shape, x: model.current.x + movement, y: model.current.y}
											}),
										$elm$core$Platform$Cmd$none);
								} else {
									return noChange;
							...
							case 'Hold':
								var _v2 = mOpt.a;
								return model.holdUse ? noChange : (_Utils_eq(model.hold, $elm$core$Array$empty) ? _Utils_Tuple2(
									_Utils_update(
										model,
										{
											current: {shape: model.next, x: 4, y: 0},
									...
									A2($elm$random$Random$generate, $author$project$Main$Next, $author$project$Main$nextGen)) : _Utils_Tuple2(
									_Utils_update(
										model,
										{
											current: {shape: model.hold, x: 4, y: 0},
											hold: model.current.shape,
											holdUse: true
										}),
									$elm$core$Platform$Cmd$none));
...
var $author$project$Main$view = function (model) {
	if (!model.startTime) {
		return model.isGameOver ? A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					$elm$html$Html$text('GameOver'),
					A2($elm$html$Html$br, _List_Nil, _List_Nil),
					$elm$html$Html$text('score : '),
					$elm$html$Html$text(
					$elm$core$String$fromInt(model.score)),
					A2($elm$html$Html$br, _List_Nil, _List_Nil),
					$elm$html$Html$text('Press any key to continue')
				])) : $elm$html$Html$text('Press any key to start');
	} else {
		var field = A2($author$project$Main$verifyFalling, model.current, model.field);
		var blockView = function (f) {
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
										...
										return A2(
											...
											_List_fromArray(
												[
													...
													A2($elm$html$Html$Attributes$style, 'letter-spacing', '-6px')
												]),
											_List_fromArray(
												[
													$elm$html$Html$text('■')
